...
- Rafael Lima (r100131093@aluno.unb.br)

Version History:
- August 3, 2017 - Rafael Lima
-- Changed print statements to remove empty value
...
class DQ:
    ...
    def __str__(self):
        ...
        for index in range(8):
            if self.q[index]!=0:
                tmpq[index] = str(self.q[index]) + tmp[index]
            else:
                tmpq[index] = 0

        resultP= ' + '.join(filter(lambda x: x!=0,tmpq[0:4]))

        if self.q[4]!=0 or self.q[5]!=0 or self.q[6]!=0 or self.q[7]!=0:
            resultD+="e( "
            resultD+= ' + '.join(filter(lambda x: x!=0,tmpq[4:8]))
            resultD+=" )"

        return ' + '.join(filter(lambda x: x!='',[resultP,resultD]))
